...
extern EL_CONST CHAR el_NIL[];

int el_user_intr = 0;
int el_PushBack=0;
int el_Pushed=0;
EL_CONST CHAR	*el_Input = el_NIL;
...
void el_ttyset(int Reset)
{
	...
	el_erase = old.c_cc[VERASE];
	el_kill = old.c_cc[VKILL];
	el_eof = old.c_cc[VEOF];
	el_intr = old.c_cc[VINTR];
	el_quit = old.c_cc[VQUIT];
#if	defined(DO_SIGTSTP)
	el_susp = old.c_cc[VSUSP];
...
void el_ttyset(int Reset)
{
	...
	el_erase = old_sgttyb.sg_erase;
	el_kill = old_sgttyb.sg_kill;

	(void)ioctl(0, TIOCGETC, &old_tchars);
	el_eof = old_tchars.t_eofc;
	el_intr = old_tchars.t_intrc;
	el_quit = old_tchars.t_quitc;

#if	defined(DO_SIGTSTP)
	(void)ioctl(0, TIOCGLTC, &old_ltchars);
	el_susp = old_ltchars.t_suspc;
#endif	/* defined(DO_SIGTSTP) */
